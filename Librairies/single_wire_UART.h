 ...
 ******************************************************************************/
#ifndef single_wire_UART_h
#define single_wire_UART_h
...
#define SW_UART_PIN_NUMBER    PD3       //!< Set pin number for communication.
...
#define TIMER_PRESCALER_CONTROL_REGISTER    TCCR2B //!< Define the timer control register according to the timer used for the UART : prescaler pour timer 2
#if (PRESCALER == 1)
  #define START_UART_TIMER()     (TIMER_PRESCALER_CONTROL_REGISTER |= (1<<CS20))  //Needs to be change if a different timer is used. Please refer to datasheet.
  #define STOP_UART_TIMER()      (TIMER_PRESCALER_CONTROL_REGISTER &= ~(1<<CS20))
#elif (PRESCALER == 8)
  #define START_UART_TIMER()     (TIMER_PRESCALER_CONTROL_REGISTER |= (1<<CS21))
  #define STOP_UART_TIMER()      (TIMER_PRESCALER_CONTROL_REGISTER &= ~(1<<CS21))
#elif (PRESCALER == 64)
  #define START_UART_TIMER()     (TIMER_PRESCALER_CONTROL_REGISTER |= (1<<CS22))
  #define STOP_UART_TIMER()      (TIMER_PRESCALER_CONTROL_REGISTER &= ~(1<<CS22))
#else
  #error PRESCALER must be set to 1 or 8 or 64
#endif
...
#define SET_UART_DIRECTION() 	( SW_UART_DDR |= (1<<SW_UART_PIN_NUMBER)) //!< output
#define CLEAR_UART_DIRECTION()	( SW_UART_DDR &= ~(1<<SW_UART_PIN_NUMBER)) //!< input 
/* UART interrupt vectors definitions. */
#define SW_UART_EXTERNAL_INTERRUPT_VECTOR       INT1_vect             //!< UART external interrupt vector. Make sure this is in accordance to the defined UART pin.
#define SW_UART_TIMER_COMPARE_INTERRUPT_VECTOR  TIMER2_COMPA_vect      //!< UART compare interrupt vector.

/* Timer macros. These are device dependent. */
//#define SET_TIMER2()     						(PRR  &= ~(1<<PRTIM2 ))  
#define CLEAR_UART_TIMER_ON_COMPARE_MATCH()     (TCCR2A = ((TCCR2A |(1<<WGM21))&(~(1<<WGM20))))                             //!< Set timer control register to clear timer on compare match (CTC).
#define SET_UART_TIMER_COMPARE_WAIT_ONE()       (OCR2A = WAIT_ONE)                                 //!< Sets the timer compare register to one period.
#define SET_UART_TIMER_COMPARE_START_TRANSMIT() (OCR2A = WAIT_ONE - (TRANSMIT_DELAY/PRESCALER))    //!< Sets the timer compare register to the correct value when a transmission is started.
#define SET_UART_TIMER_COMPARE_START_RECEIVE()  (OCR2A = WAIT_ONEHALF - (RECEIVE_DELAY/PRESCALER)) //!< Sets the timer compare register to the correct value when a reception is started.
#define CLEAR_UART_TIMER()                      (TCNT2 = 0x00)
#define ENABLE_UART_TIMER_INTERRUPT()           (TIMSK2 |= (1<<OCIE2A))
#define DISABLE_UART_TIMER_INTERRUPT()          (TIMSK2 &= ~(1<<OCIE2A))
#define CLEAR_UART_TIMER_INTERRUPT_FLAG()       (TIFR2 |= (1<<OCF2A)) // ? bien A

/* External interrupt macros. These are device dependent. */
//#define INITIALIZE_UART_EXTERNAL_INTERRUPT()    (EICRA |= (1<<ISC11))   //< Sets falling edge of INT0 generates interrupt.
//#define ENABLE_UART_EXTERNAL_INTERRUPT()        (EIMSK |= (1<<INT1))
//#define DISABLE_UART_EXTERNAL_INTERRUPT()       (EIMSK &= ~(1<<INT1))
#define CLEAR_UART_EXTERNAL_INTERRUPT_FLAG()    (EIFR = (1<<INTF1))

/* Status register defines. */
#define SW_UART_TX_BUFFER_FULL        4     //!< Set > if data is ready to be sent from the Tx buffer. (pour envoyer + de 2 sequences d'affilé)
...
int SW_UART_Receive(void);      //!< Receive one byte.
uint8_t SW_UART_ReceiveBytes(uint8_t  *buffer, uint8_t length);
