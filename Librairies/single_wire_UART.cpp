...
//#include <interrupt.h>              //The __enable_interrupt() intrinsic.
...
static volatile uint8_t UART_Tx_buffer_count;     //!< Number of data in buffer (1to3)
...
static volatile uint8_t  UART_Tx_buffer[4];   //!< Transmission buffer.
...
void SW_UART_Enable(void)
{
 //Tri-state communication pin.
  ...
  attachInterrupt(digitalPinToInterrupt(3), SW_interruptHandler, FALLING);
}
...
void SW_UART_Transmit(uint8_t data)
{
  ...
  UART_Tx_buffer_count +=1;
  UART_Tx_buffer[UART_Tx_buffer_count-1] = data;
...
ISR(SW_UART_TIMER_COMPARE_INTERRUPT_VECTOR)
{
  ...
  else
  {
    ...
    else
    {
      ...
      { 
		//*UART_Tx_buffer += sizeof(uint8_t);
		for(a_i = 0; a_i <= 2; a_i++){
			UART_Tx_buffer[a_i]=UART_Tx_buffer[a_i+1];
		}
        UART_Tx_data = UART_Tx_buffer[0];
		UART_Tx_buffer_count-=1;
		if (UART_Tx_buffer_count == 1) {
        CLEAR_FLAG( SW_UART_status, SW_UART_TX_BUFFER_FULL );
		}
